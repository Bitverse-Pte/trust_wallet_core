...
class HyperEvmTestnetExample extends BaseExample {
  final HDWallet wallet;

  const HyperEvmTestnetExample(this.wallet, {Key? key}) : super('HyperEVMTestnet', key: key);

  @override
  _HyperEvmTestnetExampleState createState() => _HyperEvmTestnetExampleState();
}

class _HyperEvmTestnetExampleState extends BaseExampleState<HyperEvmTestnetExample> {

  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeHyperEVMTestnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(10000998)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(10000998).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(10000998).data(), "", "123", 10000998);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(10000998).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 10000998);
    ...
    final privatekey = widget.wallet.getKey(10000998, "m/40'/60'/0'/0/0");
