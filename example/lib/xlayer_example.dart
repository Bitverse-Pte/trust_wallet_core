...
class XlayerExample extends BaseExample {
  final HDWallet wallet;

  const XlayerExample(this.wallet, {Key? key}) : super('XLayer', key: key);

  @override
  _XlayerExampleState createState() => _XlayerExampleState();
}

class _XlayerExampleState extends BaseExampleState<XlayerExample> {

  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeXLayer;
  ...
  void test(){
    logger.d("address ${widget.wallet.getAddressForCoin(10000196)}");
    logger.d("mnemonic = ${widget.wallet.mnemonic()}");
    print(widget.wallet.mnemonic());

    String privateKeyHex = hex.encode(widget.wallet.getKeyForCoin(10000196).data());
    logger.d("privateKeyHex = $privateKeyHex");
    logger.d("seed = ${hex.encode(widget.wallet.seed())}");
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(10000196).data(), "", "123", 10000196);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(10000196).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 10000196);

    final privateKey = widget.wallet.getKey(10000196, "m/40'/60'/0'/0/0");
