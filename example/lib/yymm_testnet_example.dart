...
class YymmTestnetExample extends BaseExample {
  final HDWallet wallet;

  const YymmTestnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _YymmTestnetExampleState createState() => _YymmTestnetExampleState();
}

class _YymmTestnetExampleState extends BaseExampleState<YymmTestnetExample> {
  @override
  void initState() {
    super.initState();
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(20585858)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(20585858).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(20585858).data(), "", "123", 20585858);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(60).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 20585858);
    ...
    final privatekey = widget.wallet.getKey(20585858, "m/40'/60'/0'/0/0");
