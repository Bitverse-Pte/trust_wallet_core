...
class BitlayerMainnetExample extends BaseExample {
  final HDWallet wallet;

  const BitlayerMainnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _BitlayerMainnetTestnetExampleState createState() => _BitlayerMainnetTestnetExampleState();
}

class _BitlayerMainnetTestnetExampleState extends BaseExampleState<BitlayerMainnetExample> {

  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeBitlayerMainnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(10200901)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(10200901).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(10200901).data(), "", "123", 10200901);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(10200901).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 10200901);
    ...
    final privatekey = widget.wallet.getKey(10200901, "m/40'/60'/0'/0/0");
