...
class AvaxFujiTestnetExample extends BaseExample {
  final HDWallet wallet;

  const AvaxFujiTestnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _AvaxFujiTestnetExampleState createState() => _AvaxFujiTestnetExampleState();
}

class _AvaxFujiTestnetExampleState extends BaseExampleState<AvaxFujiTestnetExample> {
  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeAvalancheFujiTestnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(10043113)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(10043113).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(10043113).data(), "", "123", 10043113);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(10043113).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 10043113);
    ...
    final privatekey = widget.wallet.getKey(10043113, "m/40'/60'/0'/0/0");
