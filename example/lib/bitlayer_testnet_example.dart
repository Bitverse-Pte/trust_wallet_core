...
class BitlayerTestnetExample extends BaseExample {
  final HDWallet wallet;

  const BitlayerTestnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _BitlayerTestnetTestnetExampleState createState() => _BitlayerTestnetTestnetExampleState();
}

class _BitlayerTestnetTestnetExampleState extends BaseExampleState<BitlayerTestnetExample> {
  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeBitlayerTestnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(10200810)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(10200810).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(10200810).data(), "", "123", 10200810);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(10200810).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 10200810);
    ...
    final privatekey = widget.wallet.getKey(10200810, "m/40'/60'/0'/0/0");
