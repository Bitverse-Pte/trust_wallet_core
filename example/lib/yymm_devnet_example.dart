...
class YymmDevnetExample extends BaseExample {
  final HDWallet wallet;

  const YymmDevnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _YymmDevnetExampleState createState() => _YymmDevnetExampleState();
}

class _YymmDevnetExampleState extends BaseExampleState<YymmDevnetExample> {
  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeYymmDevnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(21585858)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(21585858).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(21585858).data(), "", "123", 21585858);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(60).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 21585858);
    ...
    final privatekey = widget.wallet.getKey(21585858, "m/40'/60'/0'/0/0");
