...
class PharosAtlanticTestnetExample extends BaseExample {
  final HDWallet wallet;

  const PharosAtlanticTestnetExample(this.wallet, {Key? key}) : super('PharosAtlanticTestnet', key: key);

  @override
  _PharosAtlanticTestnetExampleState createState() => _PharosAtlanticTestnetExampleState();
}

class _PharosAtlanticTestnetExampleState extends BaseExampleState<PharosAtlanticTestnetExample> {

  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypePharosAtlanticTestnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(10688689)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(10688689).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(10688689).data(), "", "123", 10688689);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(10688689).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 10688689);
    ...
    final privatekey = widget.wallet.getKey(10688689, "m/40'/60'/0'/0/0");
