...
class SolanaExample extends BaseExample {
  final HDWallet wallet;

  const SolanaExample(this.wallet, {Key? key}) : super('Solana', key: key);

  @override
  _SolanaExampleState createState() => _SolanaExampleState();
}

class _SolanaExampleState extends BaseExampleState<SolanaExample> {
  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeSolana;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(coinId)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(coinId).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(coinId).data(), "", "123", coinId);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(60).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, coinId);
    ...
    final privakye = widget.wallet.getKey(118, "m/44'/501'/0'");
