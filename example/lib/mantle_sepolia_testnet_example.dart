...
class MantleSepoliaTestnetExample extends BaseExample {
  final HDWallet wallet;

  const MantleSepoliaTestnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _MantleSepoliaTestnetExampleState createState() => _MantleSepoliaTestnetExampleState();
}

class _MantleSepoliaTestnetExampleState extends BaseExampleState<MantleSepoliaTestnetExample> {
  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeMantleSepoliaTestnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(10005003)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(10005003).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(10005003).data(), "", "123", 10005003);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(10005003).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 10005003);
    ...
    final privatekey = widget.wallet.getKey(10005003, "m/40'/60'/0'/0/0");
