...
class ZIGChainMainnetExample extends BaseExample {
  final HDWallet wallet;

  const ZIGChainMainnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _ZIGChainMainnetExampleState createState() => _ZIGChainMainnetExampleState();
}

class _ZIGChainMainnetExampleState extends BaseExampleState<ZIGChainMainnetExample> {
  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeZIGChain;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(20006868)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(20006868).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(20006868).data(), "", "123", 20006868);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(60).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 20006868);
    ...
    final privatekey = widget.wallet.getKey(20006868, "m/40'/60'/0'/0/0");
