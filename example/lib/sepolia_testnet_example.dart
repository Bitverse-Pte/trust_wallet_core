...
class SepoliaTestnetExample extends BaseExample {
  final HDWallet wallet;

  const SepoliaTestnetExample(this.wallet, {Key? key}) : super('Cosmos', key: key);

  @override
  _SepoliaTestnetExampleState createState() => _SepoliaTestnetExampleState();
}

class _SepoliaTestnetExampleState extends BaseExampleState<SepoliaTestnetExample> {
  @override
  void initState() {
    super.initState();
    int coinId = TWCoinType.TWCoinTypeSepoliaTestnet;
    ...
    /// 也可以通过coinId直接获取地址
    logger.d("address ${widget.wallet.getAddressForCoin(11155111)}");
  ...
  void test(){
    ...
    String privateKeyhex = hex.encode(widget.wallet.getKeyForCoin(11155111).data());
    ...
    final a = StoredKey.importPrivateKey(widget.wallet.getKeyForCoin(11155111).data(), "", "123", 11155111);
    logger.d("keystore a = ${a?.exportJson()}");

    final publicKey = widget.wallet.getKeyForCoin(11155111).getPublicKeySecp256k1(false);
    final anyAddress = AnyAddress.createWithPublicKey(publicKey, 11155111);
    ...
    final privatekey = widget.wallet.getKey(11155111, "m/40'/60'/0'/0/0");
